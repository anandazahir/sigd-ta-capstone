<div class="bg-primary rounded-4 shadow p-3 my-3 position-relative" style="height: auto;">
    <div class=" container ">
        <div class="row justify-content-between p-0 m-0">
            <h2 class="text-white fw-semibold col-lg-9 m-0 p-0">Entry Data</h2>
            <a href="#" class="btn btn-info p-1 col-lg-2 mt-3 mt-lg-0">
                <i class="fa-solid fa-circle-plus text-white fa-xl mx-1"></i>
                <span class="fw-semibold fs-6">Tambah Peti Kemas</span>
            </a>
        </div>

        <div class="bg-white mt-3 p-2 rounded-4 shadow scroll" style="height: 25rem;">
            <table class="table-variations-3 table-responsive text-center">
                <thead>
                    <tr>
                        <th scope="col" class="fw-semibold">No Peti Kemas</th>
                        <th scope="col" class="fw-semibold">Size & Type</th>
                        <th scope="col" class="fw-semibold">Cetak SPK</th>
                        <th scope="col"></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="text-center">
                            555555
                        </td>
                        <td class="text-center">
                            40'FT
                        </td>
                        <td class="text-center">
                            <span class="bg-success text-white p-1 rounded-2 fs-6">SUDAH CETAK</span>
                        </td>
                        <td class="text-center">
                            <div class="btn-group gap-2 mx-auto">
                                <a class="btn btn-info text-white rounded-3"> <i class="fa-solid fa-pen-to-square fa-lg my-1"></i></a>
                                <button class="btn btn-danger text-white rounded-3"> <i class="fa-solid fa-trash-can fa-lg my-1"></i></button>
                            </div>
                        </td>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>